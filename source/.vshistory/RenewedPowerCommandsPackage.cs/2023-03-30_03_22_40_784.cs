	...
	public sealed class RenewedPowerCommandsPackage : AsyncPackage
	{
		...
		protected override async Task InitializeAsync(CancellationToken cancellationToken, IProgress<ServiceProgressData> progress)
		{
			...
			_saveCommandInterceptors = new List<CommandInterceptor>
			{
				...
				new CommandInterceptor(this.Dte).SetCommandId(VSConstants.VSStd97CmdID 331),
				new CommandInterceptor(this.Dte).SetCommandId(VSConstants.VSStd97CmdID.SaveAs),
				new CommandInterceptor(this.Dte).SetCommandId(VSConstants.VSStd97CmdID 226),
				new CommandInterceptor(this.Dte).SetCommandId(VSConstants.VSStd97CmdID 224)
			};
