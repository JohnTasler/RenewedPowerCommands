	...
	internal class CollapseProjectsCommand : DynamicCommand
	{
		...
		protected override bool CanExecute(OleMenuCommand command)
		{
			...
			if (base.CanExecute(command))
			{
				...
				if (project != null)
				{
					...
					return project.IsKind("{66A26720-8FB5-11D2-AA7E-00C04F688DDE}")
		...
		private static void OnExecute(object sender, EventArgs e)
		{
			...
			else
			{
				...
				SolutionExplorer.GetItem(DynamicCommand.Dte.Solution.Properties.Item("Name").Value.ToString()).Select(vsUISelectionType.vsUISelectionTypeSelect);
			}
		}
		...
		private static void PerformCollapse(UIHierarchyItem project)
		{
			...
			if (DTEHelper.IsProjectNode(project) && project.Object is ProjectItem projectItem && projectItem.ContainingProject.IsKind("{66A26720-8FB5-11D2-AA7E-00C04F688DDE}") && project.UIHierarchyItems.Expanded)
			{
				project.Select(vsUISelectionType.vsUISelectionTypeSelect);
