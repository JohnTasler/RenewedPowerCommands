...
namespace Tasler.RenewedPowerCommands.Commands
{
	...
	internal class RemoveSortUsingsCommand : DynamicCommand
	{
		...
		private static bool IsAtLeastOneCSharpProject()
		{
			...
			return new ProjectIterator(DynamicCommand.Dte.Solution).FirstOrDefault(p => p.IsKind("{FAE04EC0-301F-11D3-BF4B-00C04F79EFBC}")) != null;
		}
		...
		private static void ProcessProject(Project project)
		{
			...
			if (project != null)
			{
				...
				var alreadyOpenFiles = source.Select(info => info.Moniker).ToList();
				string fileName;

				Func<string, bool> predicate = f => f.Equals(f, StringComparison.OrdinalIgnoreCase));

				foreach (var item in new ProjectItemIterator(project.ProjectItems).Where(i => i.FileCodeModel != null))
				{
