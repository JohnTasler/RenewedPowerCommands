...
namespace Tasler.RenewedPowerCommands.Commands
{
	...
	internal class RemoveSortUsingsCommand : DynamicCommand
	{
		...
		private static void OnExecute(object sender, EventArgs e)
		{
			...
			if (Dte.SelectedItems.Item(1).Project?.Kind == "{FAE04EC0-301F-11D3-BF4B-00C04F79EFBC}")
			{
				RemoveSortUsingsCommand.ProcessProject(project);
				return;
			}
			foreach (var proj in new ProjectIterator(Dte.Solution.Where(p => p.Kind == "{FAE04EC0-301F-11D3-BF4B-00C04F79EFBC}"))
			{
