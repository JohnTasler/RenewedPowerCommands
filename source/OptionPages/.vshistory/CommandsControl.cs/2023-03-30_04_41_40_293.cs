...
namespace Tasler.RenewedPowerCommands.OptionPages
{
	public partial class CommandsControl : UserControl
	{
		...
		{
			this._commandManagerService = this.OptionPage.Site.GetService<SCommandManagerService, ICommandManagerService>();
			this._items = (from command in this._commandManagerService.GetRegisteredCommands()
			...
			_gridVisibility.DataSource = this._items;
